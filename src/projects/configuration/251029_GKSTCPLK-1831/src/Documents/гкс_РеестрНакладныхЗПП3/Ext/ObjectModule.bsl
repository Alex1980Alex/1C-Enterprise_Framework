...
Функция ТекстЗапросаПолучитьФормулыПоказателейИУслуг()
	...
	|ГДЕ
	|	РезультатыНормативнойСертификации.ТочкаМаршрута = &ТочкаМаршрута
	|	И РезультатыНормативнойСертификации.Номенклатура = &Номенклатура
	|	И
	|		РезультатыНормативнойСертификации.НазначениеИспользованияКачества = ЗНАЧЕНИЕ(Перечисление.гкс_НазначенияИспользованияКачества.ПриемкаКомпозит)
	|СГРУППИРОВАТЬ ПО
