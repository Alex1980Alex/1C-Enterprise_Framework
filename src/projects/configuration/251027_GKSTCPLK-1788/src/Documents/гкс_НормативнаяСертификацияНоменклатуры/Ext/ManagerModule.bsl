...
Функция ТекстЗапросаТаблицаЦБТ_РезультатыНормативнойСертификацииНоменклатуры(Запрос, ТекстыЗапроса, Регистры)
	...
	|	КОНЕЦ КАК ПериодПо,
	|	ЦБТ_НормативнаяСертификацияНоменклатуры.гкс_НазначениеИспользованияКачества КАК НазначениеИспользованияКачества,
	|	ЦБТ_НормативнаяСертификацияНоменклатуры.гкс_ВидПеревозки КАК ВидПеревозки,
...
Процедура ЗаполнитьТочкуМаршрутаВНормативнойСертификации(Параметры = Неопределено) Экспорт
	...
	Запрос.Текст =	
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РезультатыСертификации.Регистратор КАК Ссылка
		|ИЗ
		|	РегистрСведений.гкс_РезультатыНормативнойСертификацииНоменклатуры КАК РезультатыСертификации
		|ГДЕ
		|	РезультатыСертификации.ТочкаМаршрута = ЗНАЧЕНИЕ(Справочник.гкс_ТочкиМаршрута.ПустаяСсылка)";  
